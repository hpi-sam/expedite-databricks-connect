...
def setJVMOrigin(spark):
    def _capture_call_site(spark_session: "SparkSession") -> str:
        """
        ...
        call_sites = [
            f"{os.path.relpath(frame.filename)}:{frame.lineno}"
    ...
    def _with_origin(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if spark is not None and hasattr(func, "__name__"):
                ...
                jvm_pyspark_origin = (
                    spark._jvm.org.apache.spark.sql.catalyst.trees.CurrentOrigin
                )
                ...
                origin = spark._jvm.org.apache.spark.sql.catalyst.trees.Origin(
                    fragment_option, call_sites_option
                )
    ...
    return [str(origin.line()), str(origin.startPosition()), wrapper]
